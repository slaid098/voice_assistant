...
def test_cancel_timer_no_active(mock_speak):
    """cancel_timer does nothing when no timer is active."""
    from voice_assistant.services.commands import cancel_timer

    cancel_timer()  # should not raise


def test_cancel_timer_active(monkeypatch, mock_speak):
    """cancel_timer cancels active timer."""
    import voice_assistant.services.commands as cmd_mod

    mock_timer = MagicMock()
    monkeypatch.setattr(cmd_mod._timer_state, "_active_timer", mock_timer)
    cmd_mod.cancel_timer()
    mock_timer.cancel.assert_called_once()


def test_handle_stop_cancels_timer(mock_speak, monkeypatch):
    """handle_simple_command('stop') cancels active timer."""
    import voice_assistant.services.commands as cmd_mod

    mock_timer = MagicMock()
    monkeypatch.setattr(cmd_mod._timer_state, "_active_timer", mock_timer)
    cmd_mod.handle_simple_command("stop", None)
    mock_timer.cancel.assert_called_once()
